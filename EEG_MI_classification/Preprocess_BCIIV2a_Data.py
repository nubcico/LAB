...
class BCIIV2aPreprocessor:
    def __init__(self, subj_ids, x_path='C:/Users/madina.kudaibergenov/Dropbox/DATASETS/BCICIV_2a_dataset/x_data/',
                 y_path='C:/Users/madina.kudaibergenov/Dropbox/DATASETS/BCICIV_2a_dataset/x_data/y_labels/',
                 band=[0.5, 40]):
        """
        Initialize the preprocessor with subject IDs, data paths, and frequency band for filtering.

        :param: subj_ids (list): List of subject IDs
        :param: x_path (str): Path to the dataset directory
        :param: y_path (str): Path to the true label directory
        :param: band (list): Bandpass filter frequency range
        """
    ...
    def preprocess_4class_dataset(self):
        ...
        test_sec_4cl = self.extract_event_sec_given_cue(test_data, 768, channels) #768 is 'start trial' label
        ...
        input_shape = (Chans, Samples) # it is actually output shape (and input shape for further classifications)
        ...
        #just renaming variables, for convenience
    ...
    def preprocess_2class_dataset(self):
        ...
        :return: x_train, x_test (lists) : train and test data segmented and filtered with shape (trials, channels, samples)
        :return: y_train, y_test (lists) : labels for train and test [0,1] with shape (trials, 1)
        """
        ...
        test_eve_unk_cue = self.extract_event_sec_given_cue(test_data, 783, channels) # 783 is 'unknown cue'
        ...
        # extracting seconds with corresponding labels, for further segmentation
        train_event_time_2cl = self.extract_2_classes(train_data, channels)

        # segment filtered dataset with an interval [1.5*250=375] and +[3.5*250=875]
...
if __name__ == '__main__':
    ...
    preprocessor = BCIIV2aPreprocessor(subj_ids=[1, 2, 3, 4, 5, 6, 7, 8, 9],
                                       x_path='C:/Users/madina.kudaibergenov/Dropbox/DATASETS/BCICIV_2a_dataset/x_data/',
                                       y_path='C:/Users/madina.kudaibergenov/Dropbox/DATASETS/BCICIV_2a_dataset/x_data/y_labels/',
                                       band=[0.5, 40])
