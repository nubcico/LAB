...
with open("trained_model\gai_test_f1_new_res2.txt", encoding='utf-8') as f:
    lines = f.readlines()
all_labels = set()
true_labels = []
temp = []
...
with open("cleaned_data2.txt", 'a', encoding='utf-8') as f2: 
    for line in new_lines:
        ...
        if '\t' in line and len(line.split('\t')) > 1:
            ...
            # print(pred_labels[:-2])
            if row_true_labels[:-3] == '':
                count2 += 1
                continue

            if pred_labels == '':
                row = row_true_labels[:-3] + '\t\n'

            else :
                row = row_true_labels[:-3] + '\t' + pred_labels[:-3] + '\n'
            f2.write(row)
            count2 += 1
