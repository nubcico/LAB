...
df2 = pd.read_json("untrained_model\dl_project_test.json", encoding='utf-8')
...
with open("cleaned_data_untrained_exp2.txt", "r", encoding='utf-8') as f:
   lines = f.readlines()
unique_label = ''
i = 0

with open("cleaned_data_sites_untrained_exp2.txt", "a", encoding='utf-8') as f2:
    for line in lines:
        ...
        row = str(all_sites[i]).strip() + '\t' + new_true[:-1].strip() + '\t' + text_split[1] #new_pred[:-1].strip() +'\n'

        f2.write(row)

df = pd.read_csv("cleaned_data_sites_untrained_exp2.txt", delimiter='\t', header=None, names=['source', 'label', 'preds'])
...
# data = list(set(grouped_data['label'].tolist() + grouped_data['label'].tolist()))
